...
class BezierTwo {
  ...
  void reset(float ax, float ay, float dist, float angleL, float angleR) {
...
class Leaf {
  ...
  float baseRatio=0.2, tipRatio=-0.4;
  float baseAngle=PI/8, tipAngle=-PI/16;
  Leaf(float x, float y, float s, float r) {
    calculate(x,y,s,r); 
  }
  void calculate(float x, float y, float s, float r) {
...
void draw() {
  ...
  rotate(PI*iteration/(1.25*period));
  fill(100);
